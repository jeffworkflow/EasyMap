...
function unit.registerJassTriggers()
	...
	local j_trg = war3.CreateTrigger(function()
		...
		if unit.j_unit(jass.GetOrderTargetUnit())  then	
			u:event_notify('单位-发布指令', u, id2order[order], unit.j_unit(jass.GetOrderTargetUnit()), player_order, order)
		else 
			--print('物品：',ac.item.j_item(jass.GetOrderTargetItem()))
			u:event_notify('单位-发布指令', u, id2order[order], jass.GetOrderTargetItem(), player_order, order)	
		end	  
...
function unit.saveDefaultUnits()
	...
	for _, u in ipairs(group) do
		...
		-- print('增加引用：',dbg.handle_ref(u.handle))
		-- print(u.unit_type)
		-- --保存 预设单位 到 全局all_unit 里面
		-- unit.init_unit(u.handle,u.onwer)
		--print_r(unit.all_units[u.handle] )
