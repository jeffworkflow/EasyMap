...
mt{
	...
	tip = [[
		|cff11ccff被动：|r
		攻击有%chance%%增加%power%敏捷，上限为人物等级*15，持续%time%s
...
function mt:on_add()
	...
	self.event = hero:event '造成伤害效果' (function(trg, damage)
		...
		if damage:is_common_attack()   then
            ...
            --增加的力量上限为人物等级*15
			if (hero:get('敏捷')-base_attack + self.power)>= self.max_power   then
