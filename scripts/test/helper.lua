...
function helper:reload()
	...
	--遍历身上的物品
	for i = 1, 6 do
		local it = self.hero:find_skill(i, '物品')
		if it then
			local name = it.name
			local affixs = it.affixs
			local slotid = it.slotid
			it:remove()
			local skl = self.hero:add_skill(name, '物品', slotid)
			if skl then
...
function helper:timer()
	...
	ac.wait(3000, function()
		...
		print('resume:',ac.clock())
		t:resume()
	end)
	ac.wait(5000, function()
		print('remove:',ac.clock())
		t:remove()
	end)
end
...
function helper:dummy(hero_name,life, playerid)
	print("创建一个英雄")
	if not playerid then
		playerid = 13
	end
	local p = player[playerid]
	p.hero = p:createHero(hero_name, ac.point(0,0,0), 270)
...
local function main()
	...
	--按下ESC来重载脚本
	-- ac.game:event '按下ESC' (function(trg,p)
	-- 	print('回调esc')
	-- 	helper.reload(p)
	-- end)
	...
	-- 	player:event_notify('按下ESC',player)
	-- end)
